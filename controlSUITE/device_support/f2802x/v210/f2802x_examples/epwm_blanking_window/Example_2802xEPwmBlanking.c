//#############################################################################
//
//  File:   f2802x_examples_ccsv4/epwm_blanking_window/Example_F2802xBlanking.c
//
...
void InitEPwm2Example()
{
    ...
    // Use EPWM1B as a reference 
    PWM_setActionQual_Zero_PwmB(myPwm2, PWM_ActionQual_Toggle);         // TOGGLE PWM2B on Zero
    ...
    PWM_setTripZoneDCEventSelect_DCAEVT1(myPwm2, PWM_TripZoneDCEventSel_DCxHL_DCxLH);               // DCAEVT1 =  DCAH low, DCAL high;  
    PWM_setDigitalCompareFilterSource(myPwm2, PWM_DigitalCompare_FilterSrc_DCAEVT1);                // DCEVTFLT Filter Source = DCAEVT1
    PWM_setDigitalCompareAEvent1(myPwm2, true, true, false, false);                                 // DCAEVT1 = DCEVTFLT, Take asynch path

    // What do we want the event (DCAEVT1) to do?
    PWM_setTripZoneState_DCAEVT1(myPwm2, PWM_TripZoneState_EPWM_High);  // force high on DCAEVT1
    
    // Blank (block) the event during this window
    // The window starts at TBCLK = 875 and is 250 TBCLK wide
    PWM_setDigitalCompareBlankingPulse(myPwm2, PWM_DigitalCompare_PulseSel_CTR0);
    PWM_setDigitalCompareFilterOffset(myPwm2, 875);       
    PWM_setDigitalCompareFilterWindow(myPwm2, 250);
    PWM_disableDigitalCompareBlankingWindow(myPwm2);        // Blanking window is disabled
