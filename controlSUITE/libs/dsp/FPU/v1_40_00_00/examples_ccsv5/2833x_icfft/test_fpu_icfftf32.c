...
float	CFFTin1Buff[CFFT_SIZE*2];					// FFT input data buffer, alignment require
                                                    // Output of ICFFT overwrites input if 
                                                    //CFFT_STAGES is ODD
#pragma	DATA_SECTION(CFFToutBuff,"CFFTdata2")
float	CFFToutBuff[CFFT_SIZE*2];					// FFT output buffer
                                                    // Output of FFT here if CFFT_STAGES is EVEN
#pragma	DATA_SECTION(CFFTmagBuff,"CFFTdata3")
float	CFFTmagBuff[CFFT_SIZE];						// FFT output magnitude buffer
#pragma	DATA_SECTION(CFFTF32Coef,"CFFTdata4") 
float	CFFTF32Coef[CFFT_SIZE];                 	// FFT Twiddle factor buffer
