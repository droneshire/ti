...
;
; DESCRIPTION: C-Callable multiplication of a complex vector (w) and the complex conjugate of a complex vector (x)
;              xconj_re[i] = x_re[i]
;              xconj_im[i] = - x_im[i]
;              y_re[i] = w_re[i]*x_re[i] - w_im[i])*xconj_im[i]
;              y_im[i] = w_re[i]*xconj_im[i] + w_im[i]*x_re[i]
; FUNCTION: 
;   extern void mpy_SP_CVxCVC(complex_float *y, const complex_float *w, const complex_float *x, const Uint16 N)
...
;---Main loop
		...
		NEGF32		R4H, R4H					;R4H = -x_im (taking the complex conjugate of x)
