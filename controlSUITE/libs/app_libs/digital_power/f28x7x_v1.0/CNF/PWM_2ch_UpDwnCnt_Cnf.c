...
void PWM_2ch_UpDwnCnt_CNF(int16 n, int16 period, int16 mode, int16 phase)
{
	...
	(*ePWM[n]).TBPRD = period/2;						// bit-period due to up-down count
	...
	if(mode == 0) // config as a Slave (Note: Phase+2 value used to compensate for logic delay)
	{
		...
		{
			(*ePWM[n]).TBPHS.bit.TBPHS = (phase-2);
	...
	(*ePWM[n]).CMPA.bit.CMPA = (period/2)/2;			// set duty 50% initially
	...
	// Counter Compare Submodule Registers
	(*ePWM[n]).CMPB.bit.CMPB = (period/2)/2;						// set duty 50% initially
