...
void PWMDRV_PSFB_PCMC_CNF(int16 n, Uint16 period, int16 SR_Enable, int16 Comp2_Prot) {
	...
	(*ePWM[n]).CMPA.bit.CMPA = period - 68;
	(*ePWM[n]).CMPB.bit.CMPB = period;
	...
	// Counter compare submodule registers
	(*ePWM[n + 1]).CMPA.bit.CMPA = period + 10; // Initial value
	(*ePWM[n + 1]).CMPB.bit.CMPB = 20; // Initial value
	...
	(*ePWM[n + 2]).TBPHS.bit.TBPHS = period - 35;
	(*ePWM[n + 2]).TBCTR = 0;
	...
	if (SR_Enable == 1) { // PWM configuration for synchronous rectification
		...
		// Counter compare submodule registers
		(*ePWM[n + 3]).CMPB.bit.CMPB = 20; // Initial value
		(*ePWM[n + 3]).CMPA.bit.CMPA = period + 10; // Initial value
	...
	(*ePWM[n + 2]).CMPA.bit.CMPA = 40; // Note: This value is based on 100 KHz switching frequency
	...
	(*ePWM[n + 2]).CMPB.bit.CMPB = 165; // Note: This value is based on 100 KHz switching frequency
