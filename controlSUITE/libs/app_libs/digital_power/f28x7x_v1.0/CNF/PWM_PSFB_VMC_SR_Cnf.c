...
// ePWM(n) init.  Note EPWM(n) is the Master
	...
	(*ePWM[n]).TBPRD = period;
	(*ePWM[n]).CMPA.bit.CMPA = period / 2; // Fix duty at 50%
...
// ePWM(n+1) init.  EPWM(n+1) is a slave
	...
	(*ePWM[n + 1]).CMPA.bit.CMPA = period / 2; // Fix duty at 50%
	(*ePWM[n + 1]).TBPHS.bit.TBPHS = 0; // zero phase initially
	(*ePWM[n + 1]).TBCTR = 0;
...
//Configure EPWM(n+3) for synchronous rectifier drive
		...
		(*ePWM[n + 3]).TBPHS.bit.TBPHS = 8; // Accounts for syncing delays down between PWM modules
		(*ePWM[n + 3]).TBCTR = 0;
		...
		(*ePWM[n + 3]).CMPA.bit.CMPA = period / 2 - 6; // Initial value
...
// SOC generation using PWM(n+1)
	...
	(*ePWM[n + 1]).CMPB.bit.CMPB = ((*ePWM[n + 1]).TBPRD) / 2; // Intialize CMPB3
