...
//
//	Description:	Project for F28069 to run of DPS Workshop Board
...
#define FREQ_STEP_MULTIPLY (float)1.059253 //10^(1/No of steps per decade(40))
...
volatile float32 Vref1 = 0;			// Output Set Voltage
volatile float32 Vout1_slew_temp = 0;		// Temp variable: used only if implementing
										// slew rate control in the slower state machine
volatile float32 Vout1SetSlewed=0;	// Slewed set point for the FB voltage loop - start from 4V
volatile float32 Vout1SetSlewed_With_Inject=0;
volatile float32 Vout1SlewRate = 0.01;			// FB Slew rate adjustment

// DECLARE THE 3P3Z OBJECT
#pragma DATA_SECTION(cntl3p3z_vars1,"cntl_var_RAM")
#pragma DATA_SECTION(cntl3p3z_coeff1,"cntl_coeff_RAM")
CNTL_3P3Z_F_VARS cntl3p3z_vars1;
CNTL_3P3Z_F_COEFFS cntl3p3z_coeff1;

volatile float32 Vout1_Read;
volatile float32 Duty_pu,Duty_pu_DC,Duty_pwm;
...
#endif //(FLASH)
	SCIA_Init(20000000, 57600); // 20000000 is the LSPCLK or the Clock used for the SCI Module
...
// Channel Selection for Cascaded Sequencer
	...
	//Period Count= 80Mhz/200Khz = 400
	PWM_1ch_CNF(1, 400,1,0);
...
//----------------------------------------------------------------------
    ...
    CNTL_3P3Z_F_VARS_init(&cntl3p3z_vars1);
    CNTL_3P3Z_F_COEFFS_init(&cntl3p3z_coeff1);

    /* 2.5 Khz Coeff */
    cntl3p3z_coeff1.Coeff_A1=(float32)1.1201983;
	cntl3p3z_coeff1.Coeff_A2=(float32)(-0.1201983);
	cntl3p3z_coeff1.Coeff_A3=(float32)(0.0);
	cntl3p3z_coeff1.Coeff_B0=(float32)(1.1951143);
	cntl3p3z_coeff1.Coeff_B1=(float32)(-1.1571392);
	cntl3p3z_coeff1.Coeff_B2=(float32)(-0.0060149);
	cntl3p3z_coeff1.Coeff_B3=(float32)(0.0);
	cntl3p3z_coeff1.IMin=(float32)(-0.1);
	cntl3p3z_coeff1.Max=(float32)(0.9);
	cntl3p3z_coeff1.Min=(float32)(0.0);
...
//====================================================================================
	...
	while(OffsetCalCounter<10000)
	{
		...
		{
			offset_Vout= (K1*offset_Vout)+K2*(Vout1R*ADC_PU_SCALING);
			OffsetCalCounter++;
...
void A0(void)
{
	...
	if(CpuTimer0Regs.TCR.bit.TIF == 1)
	{
		...
		SerialCommsTimer++;		// used by DSP280x_SciCommsGui_32bit.c Uncomment for FLASH w/GUI
	}
...
// BoxCar Averages - Input Raw samples into History arrays
//----------------------------------------------------------------   
	Hist_Vout1[HistPtr] = Vout1R-(offset_Vout*4096.0); // Raw ADC result (Q12)
...
// Voltage setting calculated:
	Vref1= Gui_VSet1*Inv_VOUT_MAX;
...
#if (INCR_BUILD==1)
	Vout1_Read = (float)Vout1R * (float)ADC_PU_SCALING;
	duty = (Uint16)((float) EPwm1Regs.TBPRD * Duty_pu );
	EPwm1Regs.CMPA.half.CMPA = duty;
//	//Read ADC and computer Fbk Value
//	Vout1_Read= _IQsat((((float32)Vout1R*ADC_PU_SCALING)-offset_Vout),1.0,0.0);
//
